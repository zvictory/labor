  ...
  return (
    <section className="grid grid-cols-1 gap-6 md:grid-cols-3">
      <Group
        title={t('seasons')}
        data={seasons}
        colors={{ spring: '#86efac', summer: '#fde68a', autumn: '#fdba74', winter: '#bae6fd' }}
        tk={(k) => t(`season.${k}`)}
      />
      <Group
        title={t('timeTitle')}
        data={time}
        colors={{ day: '#fde68a', night: '#312e81' }}
        tk={(k) => t(`time.${k}`)}
      />
      <Group
        title={t('loveTitle')}
        data={love}
        colors={{ love: '#e11d48', like: '#f43f5e', dislike: '#a8a29e', hate: '#1c1917' }}
        tk={(k) => t(`love.${k}`)}
      />
      <p className="text-muted-foreground text-xs md:col-span-3">
        {t('basedOn', { count: votesCount })}
      </p>
    </section>
  );
};
      ...
      <ul className="space-y-2">
        {(Object.keys(data) as K[]).map((k) => (
          <li key={k} className="flex items-center gap-2">
            <span className="text-muted-foreground w-20 text-sm">{tk(k)}</span>
            <div className="relative h-2 flex-1 overflow-hidden">
              <div
                className="absolute inset-y-0 left-0"
                style={{ width: `${(data[k] / max) * 100}%`, backgroundColor: colors[k] }}
                aria-hidden
              />
            </div>
