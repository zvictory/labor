...
const groupByFamily = (notes: readonly NoteDTO[]): Map<string, NoteDTO[]> => {
...
const NoteCard = ({
  ...
  const colorFallback = note.color_hex ? (
  ...
  const localNotes = ['bergamot', 'black-tea', 'musk', 'rose', 'sandalwood', 'vetiver'];
  const hasLocal = localNotes.includes(note.slug);
  const iconUrl = hasLocal ? `/notes/${note.slug}.png` : note.icon_url;
