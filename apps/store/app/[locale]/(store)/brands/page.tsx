...
const getCleanBrandSlug = (slug: string): string => {
  const knownSlugs = Object.keys(BRAND_LOGOS);
  for (const known of knownSlugs) {
    if (slug.includes(known)) {
      return known;
    }
  }
  
  if (slug.includes('creed')) return 'creed';
  if (slug.includes('dior')) return 'dior';
  if (slug.includes('chanel')) return 'chanel';
  if (slug.includes('armani')) return 'armani';
  if (slug.includes('kurkdjian')) return 'maison-francis-kurkdjian';
  
  return slug;
};

const getCleanBrandName = (slug: string, name: string): string => {
  if (slug.includes('creed')) return 'Creed';
  if (slug.includes('dior')) return 'Dior';
  if (slug.includes('chanel')) return 'Chanel';
  if (slug.includes('armani')) return 'Giorgio Armani';
  if (slug.includes('louis-vuitton')) return 'Louis Vuitton';
  if (slug.includes('kurkdjian')) return 'Maison Francis Kurkdjian';
  if (slug.includes('escentric')) return 'Escentric Molecules';
  if (slug.includes('kilian')) return 'By Kilian';
  if (slug.includes('parfums-de-marly')) return 'Parfums de Marly';
  if (slug.includes('orto-parisi')) return 'Orto Parisi';
  if (slug.includes('penhaligon')) return "Penhaligon's";
  if (slug.includes('jo-malone')) return 'Jo Malone London';
  if (slug.includes('haute-fragrance-company') || slug.includes('hfc')) return 'Haute Fragrance Company';
  if (slug.includes('oakcha')) return 'Oakcha';
  
  return name.replace(/\s+(perfume|cologne|cologne\s+-\s+a\s+new\s+fragrance|perfume\s+-\s+a\s+new\s+fragrance|perfume\s+-\s+a\s+fragrance|cologne\s+-\s+a\s+fragrance|-\s+a\s+fragrance|-\s+a\s+new\s+fragrance).*$/i, '').trim();
};
...
const BrandLogo = ({
  ...
  const slug = getCleanBrandSlug(rawSlug);
  const name = getCleanBrandName(rawSlug, rawName);
        ...
        return (
          <svg viewBox="0 0 200 65" fill="none" xmlns="http://www.w3.org/2000/svg" className={svgCls}>
            <g fill="currentColor">
              ...
              <path d="M 68 16 L 82 16 C 88 16 91 19 91 23.5 C 91 27.5 88.5 29.5 84 30 L 91.5 41 L 85.5 41 L 78.5 30.5 L 78.5 30.5 L 74 30.5 L 74 41 L 68 41 C 68 41 68 16 68 16 Z M 74 20 L 74 26.5 L 81.5 26.5 C 84 26.5 85 25.5 85 23.5 C 85 21.5 84 20.5 81.5 20.5 Z" />
...
export default async function BrandsPage({ params }: Props) {
  ...
  try {
    const rawBrands = await getBrands(locale);
    const brandMap = new Map<string, BrandDTO>();
    
    for (const b of rawBrands) {
      const cleanSlug = getCleanBrandSlug(b.slug);
      const cleanName = getCleanBrandName(b.slug, b.name);
      
      const existing = brandMap.get(cleanSlug);
      if (existing) {
        existing.product_count += b.product_count;
      } else {
        brandMap.set(cleanSlug, {
          slug: cleanSlug,
          name: cleanName,
